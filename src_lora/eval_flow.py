...
def eval(
    ...
    def execute_chunk(carry, _):
        ...
        pred_action_chunk = policy.action(obs, action_chunk, config.inference_delay, config.num_flow_steps)
    ...
    action_chunk = policy.action(obs, init_action_chunk, config.inference_delay, config.num_flow_steps, init=True)  # [batch, horizon, action_dim]
...
def main(
    ...
    if config.seq == "lr=3e-4":
        ...
        level_paths=("worlds/l/hard_lunar_lander.json",
                     "worlds/l/trampoline.json", )
    elif config.seq == "all":
        pass
    ...
    for level_path in level_paths:
        ...
        with (log_dirs[config.step] / "policies" / f"{level_name}.pkl").open("rb") as f:
            ...
            live_state_dicts.append(state_dict.get('params'))
            ema_state_dicts.append(state_dict.get("ema_params"))
    ...
    live_state_dicts = jax.device_put(jax.tree.map(lambda *x: jnp.array(x), *live_state_dicts))
    ema_state_dicts = jax.device_put(jax.tree.map(lambda *x: jnp.array(x), *ema_state_dicts))
    if config.weak_step is not None:
    ...
    print(f"EMA Usage: {config.use_ema}")
    ...
    def _eval(config: EvalConfig, rng: jax.Array, level: kenv_state.EnvState, live_state_dict, ema_state_dict, weak_state_dict):
        policy = _model.LoRAFlowPolicy(
            obs_dim=obs_dim,
        ...
        if config.use_ema:
            state.replace_by_pure_dict(ema_state_dict)
        else:
            state.replace_by_pure_dict(live_state_dict)
    ...
    for inference_delay in [0, 1, 2, 3, 4]:
        for execute_horizon in range(max(1, inference_delay), 8 - inference_delay + 1):
            ...
            out = jax.device_get(_eval(c, rngs, levels, live_state_dicts, ema_state_dicts, weak_state_dicts))
